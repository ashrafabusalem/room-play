...
class GiftTest extends TestCase{use RefreshDatabase;
 public function test_gift_transfer_is_atomic_and_append_only():void{
  ...
  $this->actingAs($sender)->postJson("/api/rooms/{$room->public_id}/gifts/{$gift->id}",$payload)->assertCreated()->assertJsonPath('balance',60);
  $this->postJson("/api/rooms/{$room->public_id}/gifts/{$gift->id}",$payload)->assertOk()->assertJsonPath('balance',60);
