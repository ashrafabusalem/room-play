...
class GiftService {
 public function send(Room $room,Gift $gift,User $sender,User $recipient):RoomGift{
  return DB::transaction(function()use($room,$gift,$sender,$recipient){
   ...
   return RoomGift::create(['room_id'=>$room->id,'gift_id'=>$gift->id,'sender_id'=>$sender->id,'recipient_id'=>$recipient->id,'price'=>$gift->price]);
  });
 }
}
